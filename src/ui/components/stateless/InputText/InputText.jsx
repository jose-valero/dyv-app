  ...
  const onChange = (e) => {
    ...
    if (!e.target.value || !inputValue) {
      ...
      if (light) {
        ...
        e.target.parentElement.previousElementSibling.classList.toggle('InputLabel-focused-light');
  ...
  return (
    <div className='input__text__wrapper'>
      ...
      <div className={`input__inner ${light ? 'light' : 'dark'}`}>
        <input value={inputValue} onChange={onChange} ref={input} className='w-100' type='text' name={label} />
      </div>
    </div>
  );
};
